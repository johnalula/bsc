...
<?php
   ...
   $activity=-1;
    foreach ($subs as $subactivity):
    if($activity != $subactivity['Sub']['activity_id']){
         $activity = $subactivity['Sub']['activity_id'];
         echo '<tr class=success><td colspan = 3>'.$subactivity['Activity']['activity'].'</td><td>'.$subactivity['Period']['period_name'].'</td></tr>';
...
?>
        ...
        <?php
            echo $this->element(
                'related_subs',
            array(
            'activity_id' => $subactivity['Activity']['id']
            )
        );
        ?>
    </div>
