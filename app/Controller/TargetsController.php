	...
	public function add() {
  	...
  	$year = $years;
		//$measures = $this->Target->Measure->find('list');
                ...
                $objecti = $this->Target->query('SELECT Objective.id, Objective.objective_name, ObjectivesOffice.objective_id FROM objectives AS Objective,  objectives_offices As ObjectivesOffice WHERE Objective.id = ObjectivesOffice.objective_id AND ObjectivesOffice.office_id = '.$this->UserAuth->getUserOfficeId());
