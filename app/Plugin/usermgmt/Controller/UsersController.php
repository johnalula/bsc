...
class UsersController extends UserMgmtAppController {
	...
	public function register() {
		...
		if (SITE_REGISTRATION) {
			$userGroups=$this->UserGroup->getGroupsForRegistration();
                       // $offices=$this->Office->getOfficesForRegistration();
	...
	public function changePassword() {
		...
		if ($this->request -> isPost()) {
			...
			if ($this->User->RegisterValidate()) {
				...
				$this->redirect('/viewUser/'.$this->UserAuth->getUserId());
			}
		}
	}
	/**
	...
	public function addUser() {
		$userGroups=$this->UserGroup->getGroups();
                $offices=$this->User->Office->getOffices();
	...
	public function editUser($userId=null) {
		if (!empty($userId)) {
			...
			$file = $this->data['User']['picture']; //put the data into a var for easy use
				...
				$file_name=time().$file['name'];
                                move_uploaded_file($file['tmp_name'], WWW_ROOT . 'img/users' . $file_name);

                                //prepare the filename for database entry
                                $this->request->data['User']['user_img'] = $file_name;
				 
                        }
				$this->User->set($this->data);
				if ($this->User->RegisterValidate()) {
					$this->User->save($this->request->data,false);
					$this->Session->setFlash(__('The user is successfully updated'. WWW_ROOT . 'img/users'));
					$this->redirect('/allUsers');
				}
			} else {
				$user = $this->User->read(null, $userId);
